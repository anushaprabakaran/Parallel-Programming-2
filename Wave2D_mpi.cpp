...
int main(int argc, char *argv[]) {
    ...
    if (max_time > 2) {
        for (int t = 2; t < max_time; t++) {
            ...
            #pragma omp parallel for schedule( static ) firstprivate(c, dtByddSquare) shared (z)     // parallelized 
            for (int i = my_rank * stripe; i < (my_rank + 1) * stripe; i++) {
                #pragma omp parallel for default (none) firstprivate(i, c, dtByddSquare, size, curr, prev, prevPrev ) shared (z)     // parallelized
                for (int j = 0; j < size; j++) {
                    // cell is on an edge
                    if (i == 0 || i == size - 1 || j == 0 || j == size - 1) {
                        z[curr][i][j] = 0.0;
